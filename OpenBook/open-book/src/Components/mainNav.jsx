...
import ProfilePics from "../assets/profilePic.jpeg";
...
function MainNav() {
  ...
  /*const menuButtons = document.querySelectorAll(".notification-ctn");
  const menus = document.querySelectorAll(".nav-sub-menu");*/
  ...
  return (
    <nav>
      <div className="notification-ctn" onClick={() => toggleMenu(0)}>
        {NotificationIcon}

        <div className={`nav-sub-menu ${openIndex === 0 ? "flex" : "hidden"}`}>
          {WalletIcon}
          <p>Connect your wallet to see notifications</p>
        </div>
      </div>

      <div className="notification-ctn" onClick={() => toggleMenu(1)}>
        <div className="profile">
          <p>John Doe</p>
          <img src={ProfilePics} alt="User Display Picture" />
        </div>

        <button
          className={`nav-sub-menu ${openIndex === 1 ? "flex" : "hidden"}`}
        >
          {WalletIcon}
